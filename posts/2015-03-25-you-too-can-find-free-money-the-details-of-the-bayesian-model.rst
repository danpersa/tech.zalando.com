...
If we know that a parcel itemw weighs :math:`X_{1j},\dots,X_{L_jj}`, and the parcel type is :math:`k_j`, then the parcel weight is a Gaussian random variable centered around the sum of the item weights and the assumed package weight :math:`\mu_{Z,k_j}`, with a variance determined by :math:`\sigma^2_{Z,k_j}`.
